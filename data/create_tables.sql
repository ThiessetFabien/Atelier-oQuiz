...
-- created_at : Date où l'on insère un enregistrement dans la base. On aurait pu mettre ce champs dans le MLD.
...
-- author_id = auteur du quiz --> les clé étrangères assurent la bonne cohérence de la BDD => impossible de créer un quiz si l'auteur précisé n'existe pas !
-- on peut l'écrire directement sur le ligne où on définit le champs, mais aussi sur une ligne a posteriori
-- exemple, en fin de table : FOREIGN KEY("author_id") REFERENCES "quiz"("id")
